...
class TestIntersectionOverUnion(unittest.TestCase):
    def setUp(self):
        ...
        self.t1_box1 = torch.tensor([0.8, 0.1, 0.2, 0.2], dtype = torch.half)
        self.t1_box2 = torch.tensor([0.9, 0.2, 0.2, 0.2], dtype = torch.half)
        self.t1_correct_iou = 1 / 7

        self.t2_box1 = torch.tensor([0.95, 0.6, 0.5, 0.2], dtype = torch.half)
        self.t2_box2 = torch.tensor([0.95, 0.7, 0.3, 0.2], dtype = torch.half)
        self.t2_correct_iou = 3 / 13

        self.t3_box1 = torch.tensor([0.25, 0.15, 0.3, 0.1], dtype = torch.half)
        self.t3_box2 = torch.tensor([0.25, 0.35, 0.3, 0.1], dtype = torch.half)
        self.t3_correct_iou = 0

        self.t4_box1 = torch.tensor([0.7, 0.95, 0.6, 0.1], dtype = torch.half)
        self.t4_box2 = torch.tensor([0.5, 1.15, 0.4, 0.7], dtype = torch.half)
        self.t4_correct_iou = 3 / 31

        self.t5_box1 = torch.tensor([0.5, 0.5, 0.2, 0.2], dtype = torch.half)
        self.t5_box2 = torch.tensor([0.5, 0.5, 0.2, 0.2], dtype = torch.half)
        self.t5_correct_iou = 1

        # (x1,y1,x2,y2) format
        self.t6_box1 = torch.tensor([2, 2, 6, 6], dtype = torch.half)
        self.t6_box2 = torch.tensor([4, 4, 7, 8], dtype = torch.half)
